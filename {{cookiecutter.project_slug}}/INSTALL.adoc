...
* Courtesy of https://github.com/alexphelps/server-error-pages[Alex Phelps] (same as above icon:hand-o-up[]) and https://github.com/AndiDittrich/HttpErrorPages[Andi Dittrich].
...
* `initialize_db.sh` -- deletes all migrations and `.pyc` files, recreates Postgres / PostGIS database (or deletes sqlite database -- not yet implemented), recreates migrations, superuser and basically lets you _start afresh_.
* `syncdb.sh` -- for backing up and restoring Postgres / PostGIS database
...
* Option to use https://www.sparkpost.com/[Sparkpost] as email backend on production, via the https://github.com/SparkPost/python-sparkpost[python sparkpost client]
...
----
cookiecutter https://github.com/engineervix/django-cookiecutter-vix
----
